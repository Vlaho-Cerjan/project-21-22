...
export default function AgGridSearchModal({
  openSearch,
  setOpenSearch,
  searchText,
  setSearchText,
  ...
  React.useEffect(() => {
    ['devices', 'media', 'recent'].forEach((item) => {
      const max = item;
      if (Object.values(seeAll).find((tempItem) => tempItem === true)) {
        return;
      }
      ...
      if (max) {
        ...
        const heightData = max === 'recent' ? 2 : tempHeightData;
