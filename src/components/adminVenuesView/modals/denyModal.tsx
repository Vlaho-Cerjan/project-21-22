...
export default function DenyModal({
  ...
  itemCount: number;
  data: VenueAdminData;
  setData: React.Dispatch<React.SetStateAction<VenueAdminData>>;
}) {
  ...
  */
  ...
  //React.useEffect(() => {
  //  setDenyNote((data && data.notes.venue_notes) || '');
  //}, [data]);
  ...
  return (
    <div>
      <Modal className="denyModal" open={open} setOpen={setOpen}>
        <ModalHeader
          title={`Deny Pending Venue${itemCount > 1 ? 's' : ''}`}
                ...
                onChange: (e) => {
                  if (data)
                    setData({
                      ...
                      notes: {
                        ...data.notes,
                        decline_reason: e.currentTarget.value,
                      },
                    });
        ...
        <ModalFooter
          setOpen={setOpen}
          backDataTestId="denyVenueBack"
          submitDataTestId="denyVenueSubmit"
          isError={isError}
