...
const fetchList = async ({
  ...
  console.log(
    `admin/venue/list?business_id=${id}&limit=${listLimit}&offset=${offset}&order_by=${orderKey}&direction=${direction}${searchText !== '' ? `&search=${searchText}` : ''}${checkedFilters && checkedFilters.length > 0 ? `&status=${checkedFilters.join(' ')}` : ''}`,
    ` fetchList `,
    `admin/group/list?business_id=${id}&limit=${listLimit}&offset=${offset}&order_by=${orderKey}&direction=${direction}${searchText !== '' ? `&search=${searchText}` : ''}${checkedFilters && checkedFilters.length > 0 ? `&status=${checkedFilters.join(' ')}` : ''}`,
  );
  ...
  const res1: {
    ...
    uri: `admin/venue/list?business_id=${id}&limit=${listLimit}&offset=${offset}&order_by=${orderKey}&direction=${direction}${searchText !== '' ? `&search=${searchText}` : ''}${checkedFilters && checkedFilters.length > 0 ? `&status=${checkedFilters.join(' ')}` : ''}`,
    auth: true,
  });
  ...
  const res2: {
    ...
    uri: `admin/group/list?business_id=${id}&limit=${listLimit - (offset === 0 ? 1 : 0)}&offset=${offset !== 0 ? offset - 1 : offset}&order_by=${orderKey}&direction=${direction}${searchText !== '' ? `&search=${searchText}` : ''}${checkedFilters && checkedFilters.length > 0 ? `&status=${checkedFilters.join(' ')}` : ''}`,
    auth: true,
  });
...
const searchList = async ({
  ...
  console.log(
    `admin/venue/search?business_id=${id}&limit=${listLimit}&offset=${offset}&order_by=${orderKey}&direction=${direction}${searchText !== '' ? `&search=${searchText}` : ''}${checkedFilters && checkedFilters.length > 0 ? `&status=${checkedFilters.join(' ')}` : ''}`,
    ` searchList `,
    `admin/group/list?business_id=${id}&limit=${listLimit}&offset=${offset}&order_by=${orderKey}&direction=${direction}${searchText !== '' ? `&search=${searchText}` : ''}${checkedFilters && checkedFilters.length > 0 ? `&status=${checkedFilters.join(' ')}` : ''}`,
  );
  ...
  const groupRes: {
    ...
    uri: `admin/group/list?business_id=${id}&limit=${listLimit}&offset=${offset}&order_by=${orderKey}&direction=${direction}${searchText !== '' ? `&search=${searchText}` : ''}${checkedFilters && checkedFilters.length > 0 ? `&status=${checkedFilters.join(' ')}` : ''}`,
    auth: true,
  });
  ...
  const response: {
    ...
    uri: `admin/venue/search?business_id=${id}&limit=${listLimit}&offset=${offset}&order_by=${orderKey}&direction=${direction}${searchText !== '' ? `&search=${searchText}` : ''}${checkedFilters && checkedFilters.length > 0 ? `&status=${checkedFilters.join(' ')}` : ''}`,
    auth: true,
  });

  if (response.data.venues) {
    const venList: (VenueAdminData & {
      groupName?: string;
      type?: string;
...
const fetchGroups = async ({
  ...
  const venuesRes: {
    ...
    uri: `admin/venue/list?business_id=${id}&limit=1&offset=0&order_by=${orderKey}&direction=${direction}${searchText !== '' ? `&search=${searchText}` : ''}${checkedFilters && checkedFilters.length > 0 ? `&status=${checkedFilters.join(' ')}` : ''}`,
    auth: true,
  });
  ...
  return {
    ...
    totalVenues: venuesRes && venuesRes.data && venuesRes.data.total,
  };
};
...
const fetchGroupList = async ({
  ...
  console.log(
    `admin/venue/list?business_id=${id}&group_id=${groupId}&limit=${listLimit}&offset=${offset}&order_by=${orderKey}&direction=${direction}${searchText !== '' ? `&search=${searchText}` : ''}${checkedFilters && checkedFilters.length > 0 ? `&status=${checkedFilters.join(' ')}` : ''}`,
    ` fetchGroupList `,
  );
  ...
  const res2: {
    ...
    uri: `admin/venue/list?business_id=${id}&group_id=${groupId}&limit=${listLimit}&offset=${offset}&order_by=${orderKey}&direction=${direction}${searchText !== '' ? `&search=${searchText}` : ''}${checkedFilters && checkedFilters.length > 0 ? `&status=${checkedFilters.join(' ')}` : ''}`,
    auth: true,
  });
...
const createVenue = async (data: VenueAdminCreateData) => {
  const res = await ClientApiRequest({
    uri: 'admin/venue',
    method: 'POST',
    auth: true,
...
/*
const updateData: (updatedData: VenueAdminData | null) => Promise<{
  success: boolean;
  data: VenueAdminData;
}> = async (updatedData) => {
...
const toggleVenues = async (venues: string[], bId: string, gId: string) => {
  const res = await ClientApiRequest({
    uri: 'admin/venue/toggle',
    method: 'PUT',
    auth: true,
    data: {
      business_id: bId,
...
const deleteVenue = async (venueId: string) => {
  const res = await ClientApiRequest({
    uri: `admin/venue?venue_id=${venueId}`,
    method: 'DELETE',
    auth: true,
  });
...
const deleteGroup = async (groupId: string) => {
  const res = await ClientApiRequest({
    uri: `admin/group?group_id=${groupId}`,
    method: 'DELETE',
    auth: true,
  });
...
export default function AdminVenues({params}: {params: {businessId: string}}) {
  const {businessId} = params;
  const venueGroupName = 'Non Grouped Venues';
  const gridRef = React.useRef<AgGridReact>(null);
  const [venueList, setVenueList] = React.useState<
    (VenueAdminData | VenueAdminGroupData)[]
  >([]);
  /*
  ...
  */
  ...
  }, [businessId, selectedSortKey, direction, filters, searchText]);
  /*
...
*/
  const searchItems = React.useMemo(
    () => async (id: string, searchTextData: string, filtersData: Filter[]) => {
      setLoading(true);
      const res = await searchList({
        id,
        orderKey: selectedSortKey || 'name',
  ...
  const onSearch = React.useMemo(
    () =>
      async (searchTextData: string, filtersData: Filter[], id?: string) => {
  ...
  const getRowsFunction = useCallback(
    async (ps: any) => {
      ...
      const searchFunc = async () => {
        const res = await searchList({
          ...
          filters,
          searchText: searchTextData,
          listLimit: (request.endRow || limit) - (request.startRow || 0),
        });
      ...
      const fetchGroupData = async () => {
        const offsetNumber = request.startRow
        ...
        if (res.success) {
          const venList: (VenueAdminGroupData & {
            groupName?: string;
            type?: string;
          })[] = [];
    ...
    [searchText, selectedSortKey, direction, filters],
  );
  ...
  */
  const columns = React.useMemo(
    ...
    [filters, searchText, venueTypes, statusData],
  );
    ...
    <div className="folderViewContainer">
      <GridListView
        ...
        saveGridSettings={(settings) => {
          ...
          dispatch(
            setGridSettings({
              adminVenuesGridSettings: settings,
            }),
          );
        }}
        onSort={onSort}
            ...
            onSubmit={async (data) => {
              ...
              if (res.success) {
                if (
                  ...
                  filters
                    ?.find((f) => f.id === 'status')
                    ?.data.filter((f) => f.id === 2 && f.checked) &&
                  gridRef &&
                  gridRef.current &&
                  gridRef.current.api
                ) {
        ...
        editMediaModal={
          <EditVenueModal
            ...
            onSubmit={async (id, reason) => {
              ...
              const itemGroup = item.data.group_id
                ? await fetchGroup(item.data.group_id)
                : null;
              if (reason === 'approve') {
                if (
                  venueList &&
                  ...
                  setVenueList(
                    venueList.filter((r) => 'id' in r && r.id !== id),
                  );
              ...
              if (item.success) {
                if (gridRef && gridRef.current) {
                  gridRef.current.api.applyServerSideTransaction({
                    route: [!itemGroup ? venueGroupName : itemGroup?.data.name],
                    update: [
                      {
                        ...item.data,
                        groupName: itemGroup?.data.name,
                        group_id: itemGroup?.data.id,
                        type: 'venue',
                      },
                    ],
                  });
