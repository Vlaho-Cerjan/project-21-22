...
import FormButton from './formButton'; // Adjust the path accordingly
...
describe('FormButton', () => {
  ...
  it('should render with default text', () => {
    render(<FormButton />, {wrapper: TestWrapper});
    expect(screen.getByText('Sign in')).toBeInTheDocument();
  });

  it('should render with a custom class', () => {
    render(<FormButton text="Sign in" className="custom" />, {
      wrapper: TestWrapper,
    });
  ...
  it('should display loading text and icon when loading', async () => {
    ...
    render(
      <FormButton text="Sign in" loadingText="Loading..." onClick={onClick} />,
      {
        wrapper: TestWrapper,
      },
    );
  ...
  it('should call setLoading on click', async () => {
    const onClick = vi.fn().mockResolvedValue('success');
    render(<FormButton text="Sign in" onClick={onClick} />, {
      wrapper: TestWrapper,
    });
    const button = screen.getByText('Sign in');
    fireEvent.click(button);
  ...
  it('should render with different variants', () => {
    const {rerender} = render(
      <FormButton text="Sign in" buttonVariant="success" />,
      {
        wrapper: TestWrapper,
      },
    );
    expect(screen.getByText('Sign in')).toHaveClass('success');
    rerender(<FormButton text="Sign in" buttonVariant="error" />);
    expect(screen.getByText('Sign in')).toHaveClass('error');
    rerender(<FormButton text="Sign in" buttonVariant="warning" />);
    expect(screen.getByText('Sign in')).toHaveClass('warning');
    rerender(<FormButton text="Sign in" buttonVariant="info" />);
