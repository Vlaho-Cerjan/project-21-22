...
> & {
  ...
  const handleClick = async () => {
    ...
    const errorEls = modal.querySelectorAll('.errorContainer');

    const filteredInputs = Array.from(inputs).filter(
      (input) =>
        input.closest('.modal')?.classList.contains(modalClass) ?? false,
    );

    const filteredErrorEls = Array.from(errorEls).filter(
      (errorEl) =>
        errorEl.closest('.modal')?.classList.contains(modalClass) ?? false,
    );
  ...
  return (
    <Button
      {...props}
      data-testid={props['data-testid'] || 'formButton'}
