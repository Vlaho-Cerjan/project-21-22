    ...
    if (node.data) {
      ...
      if (
        ...
        if (node.data.group_id) {
          node.data.group_id =
            newParentPath === 'non_grouped' ? null : newParentPath;
        } else if (node.data.venue_id) {
          node.data.venue_id =
            newParentPath === 'non_grouped' ? null : newParentPath;
        }
...
const isSelectionParentOfTarget = (selectedNode: RowNode, targetNode: any) => {
...
const arePathsEqual = (
  ...
  //console.log(path1, path2, 'arePathsEqual');
  ...
  if (path2.length === 1) {
    ...
    const path2Path =
      path2[0]?.data.orgHierarchy ||
      path2[0]?.data.group_id ||
      path2[0]?.data.venue_id ||
      null;
    //console.log(path1Path, path2Path, 'arePathsEqual');
    if (typeof path2Path === 'string' || path2Path === null) {
      return path1Path === path2Path;
    } else if (Array.isArray(path2Path)) {
      return path1Path === path2Path[path2Path.length - 1];
    }
  }
  const paths: {
    id: string;
  ...
  for (let i = 0; i < path2.length; i++) {
    if (typeof path1 === 'string') {
      const path1Path = path1 === 'non_grouped' ? null : path1;
      paths.push({
        id: path2[i]?.data.id || '',
        equal:
          path1Path === (path2[i]?.data.group_id || path2[i]?.data.venue_id),
      });
    } else {
      paths.push({
        id: path2[i]?.data.id || '',
        equal: path1[path1.length - 1] === path2[i]?.data.groupName,
      });
    }
  }
  return paths;
...
const SsrmListView = forwardRef<AgGridReact, ListViewProps>(
  function ListView(props, ref) {
      ...
      () => ({
        'hover-over': (params: any) => {
          ...
          if (
            ...
            dragItem &&
            (!dragItem.some(
              (di) => (di.group_id || di.venue_id) !== params.data.id,
            ) ||
              (!dragItem.some((di) => di.group_id) &&
          ...
          if (
            ...
            !dragItem.some(
              (di) =>
                di.group_id !== params.data.group_id ||
                di.venue_id !== params.data.venue_id ||
                (di.group_id === null &&
                  params.data.group_id !== 'non_grouped'),
            )
          )
            return false;
    ...
    const getFileCellRenderer = React.useCallback(() => {
      ...
      (
        ...
        */
        if (
          ...
          data.dragItem &&
          !data.dragItem.some(
            (di) => (di.group_id || di.venue_id) !== newPotentialParent.data.id,
          )
        ) {
    ...
    const cellClassFnc = ({
      ...
      if (params.data) {
        if (
          ...
          dragItem?.some(
            (di) =>
              (di.group_id && di.group_id !== params.data.id) ||
              (di.venue_id && di.venue_id !== params.data.id) ||
              (di.group_id === null && params.data.id !== 'non_grouped'),
          )
          /*
            ||
            (params.node.data &&
              ...
              dragItem.some(
                (di) =>
                  di.group_id !== params.data.group_id ||
                  (di.group_id === null && params.data.group_id !== null),
              ))
              */
        ) {
          ...
          ? columnDefsData.map((colDefData, index) => {
              return {
                ...
                cellClass: (params: any) => {
                  return cellClassFnc({
                    params,
                    i: gridOptions?.selection ? index + 1 : index,
                    dragActive: isDragActive,
                    uploadActive: isUploadActive,
                    cDDL:
                      (gridOptions?.selection ? 1 : 0) + columnDefsData.length,
                  });
                },
              };
            })
    ...
    }, [cellClassRules, isDragActive, isUploadActive]);
        ...
        groupColumnDefData
          ? {
              ...
              rowDrag: hardcodedTopFolders
                ? (params: RowDragCallbackParams) => {
                    if (
                      ...
                      params.node.data &&
                      !(params.data.group_id || params.data.venue_id)
                    )
                      return false;
              ...
              cellRendererParams: {
                // checkbox: true,
                //suppressCount: true,
                ...groupColumnDefData.cellRendererParams,
    ...
    */
    ...
    const onRowDragEnd = useCallback(
      async (event: any) => {
        ...
        const movingData: RowNode[] = event.nodes;
        const oldCopy: GridData[] = event.nodes.map((node: any) => node.data);
        const oldData: RowNode[] = cloneDeep(event.nodes);
        ...
        const newParentName: string = potentialParent.data
          ? potentialParent.data.name ||
            potentialParent.data.title ||
            potentialParent.data.business_name
          : '';

        const equalPaths = arePathsEqual(newParentPath, movingData || null);

        if (typeof equalPaths !== 'boolean') {
          equalPaths.forEach((needToChange) => {
            const findData = movingData.find(
              (data) => data.id === needToChange.id,
            );

            //console.log(findData, needToChange, 'findData');

            if (findData && needToChange.equal) {
              movingData.splice(movingData.indexOf(findData), 1);
              oldCopy.splice(oldCopy.indexOf(findData.data), 1);
              oldData.splice(oldData.indexOf(findData), 1);
            }
          });
        }

        const needToChangeParent =
          (typeof equalPaths === 'boolean' && !equalPaths) ||
          (typeof equalPaths !== 'boolean' &&
            equalPaths.some((ep) => !ep.equal));

        movingData.filter((data) => {
          const invalidModeData = isSelectionParentOfTarget(
            data,
            potentialParent,
          );
          //console.log(invalidModeData, 'invalidModeData');
          return !invalidModeData;
        });

        const oldPaths = [
          ...new Set(
            movingData.map((md) => md.data.groupName || md.data.orgHierarchy),
          ),
        ];

        const invalidMode = movingData.length === 0;
        ...
        */
        if (needToChangeParent && !invalidMode && gridRef && gridRef.current) {
          ...
          if (
            typeof onDragDropFunction !== 'undefined' &&
            updatedRows &&
            updatedRows.length > 0
          ) {
            const res = await onDragDropFunction(updatedRows);
            //const res = {success: true};
            if (res.success) {
              oldPaths.forEach((oldPath) => {
                const dataToRemove = oldCopy.filter(
                  (od) =>
                    od.groupName === oldPath || od.orgHierarchy === oldPath,
                );
                //console.log(oldData, oldPath, dataToRemove, 'dataToRemove');
                if (gridRef && gridRef.current && gridRef.current.api)
        ...
        >
          {
            <AgGridReact
              ...
              selectionColumnDef={selectionDefs}
              sideBar={sideBar}
