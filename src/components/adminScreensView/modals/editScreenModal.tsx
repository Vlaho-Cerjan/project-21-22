...
const getData = async (id: string) => {
  const res = await fetch(`/admin/businesses/screens/items/${id}`);
...
export default function EditScreenModal({
  ...
  const [formData, setFormData] = React.useState<UpdateAdminScreen>({
    screen_id: '',
    name: '',
    settings: {
      genre_edm: 0,
      ...
      if (id) {
        ...
        if (res.success) {
          setFormData({
            ...
            dooh_enabled: res.data.dooh_enabled,
            status: res.data.status,
            shipping_city: '',
            notes: {
              screen_notes: res.data.notes.screen_notes,
            },
          });
        }
      }
