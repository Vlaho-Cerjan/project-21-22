...
export default function DenyModal({
  ...
  itemCount: number;
  data: RegistrationList | null;
  setData: React.Dispatch<React.SetStateAction<RegistrationList | null>>;
}) {
  ...
  */
  ...
  React.useEffect(() => {
    setDenyNote((data && data.notes.registration_notes) || '');
  }, [data]);
  ...
  return (
    <div>
      <Modal className="denyModal" open={open} setOpen={setOpen}>
        <ModalHeader
          title={`Deny Pending Registration${itemCount > 1 ? 's' : ''}`}
